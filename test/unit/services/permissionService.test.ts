...
describe('PermissionService', () => {
  ...
  beforeEach(() => {
    ...
    getUserByIdSpy = jest
      .spyOn(userServiceModule.userService, 'getUserById')
  ...
  describe('hasPermission', () => {
    it('should return false if user does not exist', () => {
      expect(permissionService.hasPermission('nonexistent', ResourceType.TOPIC, Action.READ)).toBe(
        false,
      );
    });
    ...
    it('should return true if user is the resource owner', () => {
      expect(
        permissionService.hasPermission('viewer1', ResourceType.TOPIC, Action.UPDATE, 'viewer1'),
      ).toBe(true);
    });

    it('should return false if strategy for the user role does not exist', () => {
      expect(permissionService.hasPermission('invalidRole', ResourceType.TOPIC, Action.READ)).toBe(
        false,
      );
    });
    ...
    it('should use the appropriate strategy based on user role', () => {
      ...
      expect(permissionService.hasPermission('admin1', ResourceType.SYSTEM, Action.UPDATE)).toBe(
        true,
      );

      // Editor can read, create, update topics but not delete them
      expect(permissionService.hasPermission('editor1', ResourceType.TOPIC, Action.CREATE)).toBe(
        true,
      );
      expect(permissionService.hasPermission('editor1', ResourceType.TOPIC, Action.READ)).toBe(
        true,
      );
      expect(permissionService.hasPermission('editor1', ResourceType.TOPIC, Action.UPDATE)).toBe(
        true,
      );
      expect(permissionService.hasPermission('editor1', ResourceType.TOPIC, Action.DELETE)).toBe(
        false,
      );

      // Viewer can only read topics
      expect(permissionService.hasPermission('viewer1', ResourceType.TOPIC, Action.READ)).toBe(
        true,
      );
      expect(permissionService.hasPermission('viewer1', ResourceType.TOPIC, Action.CREATE)).toBe(
        false,
      );
      expect(permissionService.hasPermission('viewer1', ResourceType.TOPIC, Action.UPDATE)).toBe(
        false,
      );
      expect(permissionService.hasPermission('viewer1', ResourceType.TOPIC, Action.DELETE)).toBe(
        false,
      );
    });
    ...
    it('should check cross-resource permissions correctly', () => {
      ...
      expect(permissionService.hasPermission('editor1', ResourceType.USER, Action.UPDATE)).toBe(
        false,
      );

      // Editor cannot access system resources
      expect(permissionService.hasPermission('editor1', ResourceType.SYSTEM, Action.READ)).toBe(
        false,
      );

      // Viewer cannot access users or system
      expect(permissionService.hasPermission('viewer1', ResourceType.USER, Action.READ)).toBe(
        false,
      );
      expect(permissionService.hasPermission('viewer1', ResourceType.SYSTEM, Action.READ)).toBe(
        false,
      );
    });
  });
